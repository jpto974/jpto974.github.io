import {user, idUser, idCust, idProd} from './constante.ems.js'
import {ouvrirPopup} from './popup.ems.js'
import {FormClass} from './form.ems.js'
import {ArrayStorage} from './storage.ems.js'
import {listCustomer} from './client.ems.js'
import {listProduct} from './produit.ems.js'
...
function save(e ,list){
    ...
    const returnFrom = e.target.parentNode.parentNode.childNodes
    let verif = true
    const valNode = returnFrom[0].childNodes
    let recupVal = []
    for(let i = 0; i < valNode.length; i++){

        if(i !== 1 || valNode.length < 4){
    ...
    list.listL.forEach(val => {
        ...
        for(let i = 0; i < recupVal.length; i++){
            ...
            if(recupVal[i] === '' || recupVal[i] === val[i]){
...
function fCustomer(){
    ...
    button.addEventListener('click', (e) => {//ajout form client
        ...
        buttonAdd.addEventListener('click', (e) => {//charge un client
            ...
            ouvrirPopup('popupc.html','popupc','width=400,height=300,menubar=no,status=no')
...
function initLigne(){
    ...
    button.addEventListener('click', (e) => {//ajout d'une ligne
        ...
        buttonAdd.addEventListener('click', (e) => {//charger une valeur
            ...
            if(inputNotNull){//si l'input n'est pas! sorti et ne rien faire
            ...
            ouvrirPopup('popupp.html','popupp','width=400,height=300,menubar=no,status=no')
